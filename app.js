// Operations & Policy Guide — shared app engine
// Renders GUIDE_DATA (data.js) into the page, injects screenshots from
// GUIDE_MEDIA (media.js), and powers smart search, review-flag tracking,
// copy-link, back-to-top, and the cross-guide switcher.
...
(function () {
  ...
  const switchGuideSlot = document.getElementById('switchGuideSlot');

  const REVIEW_MARKERS = ['VALIDATION REQUIRED', 'PARTIALLY VALIDATED', 'CURRENT POLICY REQUIRED', 'TO BE VALIDATED', 'NOT YET VALIDATED'];
  function isFlagged(html) {
    if (!html) return false;
    return REVIEW_MARKERS.some(m => html.indexOf(m) !== -1);
  }

  // ---------------------------------------------------------------
  // 0. Cross-guide switcher
  // ---------------------------------------------------------------
  if (switchGuideSlot && typeof OTHER_GUIDE !== 'undefined' && OTHER_GUIDE && OTHER_GUIDE.url) {
    const a = document.createElement('a');
    a.className = 'switch-guide';
    a.href = OTHER_GUIDE.url;
    a.innerHTML = `${escapeHtml(OTHER_GUIDE.label)} <span aria-hidden="true">→</span>`;
    switchGuideSlot.appendChild(a);
  }

  // ---------------------------------------------------------------
  // 1. Render sidebar navigation
  // ---------------------------------------------------------------
  const flaggedItems = []; // {anchorId, breadcrumb, title}

  GUIDE_DATA.forEach((cat, ci) => {
    const wrap = document.createElement('div');
    wrap.className = 'nav-cat open';
    ...
    cat.items.forEach(sec => {
      ...
      a.innerHTML = escapeHtml((sec.number ? sec.number + '. ' : '') + sec.title) +
        (secFlagged ? ' <span class="nav-flag-dot" title="Contains items flagged for review"></span>' : '');
      li.appendChild(a);
      ul.appendChild(li);
    });
    wrap.appendChild(ul);
  ...
  GUIDE_DATA.forEach(cat => {
    ...
    cat.items.forEach(sec => {
      ...
      html += `<h2 class="section-title">${escapeHtml(sec.title)}${flagBadge(secFlagged)}${copyLinkBtn(sec.id)}</h2>`;
      ...
      sec.blocks.forEach(b => {
        ...
        html += `<h3 class="block-title">${escapeHtml(b.heading)}${flagBadge(bFlagged)}${copyLinkBtn(b.id)}</h3>`;
        html += b.html;
  ...
  // ---------------------------------------------------------------
  // 1b. Auto-link emails, URLs & Smartsheet links; make them copyable
  // ---------------------------------------------------------------
  ...
  function linkifyContent(root) {
    ...
    nodes.forEach(node => {
      ...
      while ((match = COMBINED_RE.exec(text))) {
        found = true;
        const urlMatch = match[1], emailMatch = match[2];
        let value = match[0];
        let trailing = '';
        if (urlMatch) {
  ...
  function copyLinkBtn(id) {
    return ` <button type="button" class="copy-link-btn" data-copy-id="${escapeAttr(id)}" title="Copy link to this policy" aria-label="Copy link to this policy">
      <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M6.5 9.5L9.5 6.5M5 11L3.5 12.5C2.4 13.6 2.4 15.4 3.5 16.5V16.5C4.6 17.6 6.4 17.6 7.5 16.5L9 15M11 5L12.5 3.5C13.6 2.4 15.4 2.4 16.5 3.5V3.5C17.6 4.6 17.6 6.4 16.5 7.5L15 9" stroke="currentColor" stroke-width="1.3" stroke-linecap="round" transform="translate(-2 -2)"/></svg>
    </button>`;
  }

  // ---------------------------------------------------------------
  // 2b. Needs-review panel (sidebar, above categories)
  // ---------------------------------------------------------------
  if (reviewPanel) {
  ...
  function renderMedia(blockId) {
    const shots = (typeof GUIDE_MEDIA !== 'undefined' && GUIDE_MEDIA[blockId]) ? GUIDE_MEDIA[blockId] : null;
  ...
  if (location.hash) {
    ...
    setActiveNav(GUIDE_DATA[0].items[0].id);
  }
})();
